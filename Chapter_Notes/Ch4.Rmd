...
Because it is the least interesting assumption to make. We're saying this thing probably has a finite variance for example. Because of what's common, we need need to justify that there is something about this process is different from a Gaussian. This has to do w/ideas about maximum entropy. The gaussian is generally a conservative assumption
...
hi ~ Normal(mu, sigma) (height is normally distributed)
mui = a + b*yi (mean height is a function of year)
a ~ Normal(91, 15) (some distribution of initial heights, I'm assuming these kids are young, but I'm going to let the variance be wide)
b ~ Normal(15, 2) <- 6" is a lot of growth every year, but I have no good estimate here.
sigma ~ Uniform(0, 63) <- I have no good guess at this
...
My alpha prior should have a mean of 120cm 
I also need to make sure by beta prior is always positive. Maybe I wouldn't want to use a normal distribution here. Should I instead use a uniform dist? I don't have a good intuition for it tbh.

4M6. Now suppose I tell you that the variance among heights for students of the same age is never more than 64cm. How does this lead you to revise your priors? 
My sigma can be set to a uniform on [0, 64]
Just want to mentioned I acually guessed 25", which is equal to 63 cm, which is pretty darn close to what they gave here.
...
1 &2. Set up the structure of the map and map it
```{r}
m4.3 <- map(
  alist(
    height ~ dnorm(mu, sigma),
...
use model m4.3 from above
```{r}
precis(m4.3)
```

3. do the simulation stuff and look at the estimates
```{r}
sim.height <- sim(m4.3, data=list(weight=heights$weight))
...
precis(m4.4)
```
for ever 10 units of weight we'd expect to gain 27 units of height
...
```{r}
#make a sequence of heights we would like to plot
weight.seq <- seq(from=0, to=50, by=1)
#simulate a bunch of heights for each weight 
mu <- link(m4.4, data = data.frame(weight=weight.seq))

#find the mean value of the simulated values
mu.mean <- apply(mu, 2, mean)
#find the HPDIs of the simulated values
mu.HPDI <- apply(mu, 2, HPDI, prob=0.89)

#now simulate (predict) heights given the values of weight
sim.height <- sim(m4.4, data=list(weight = weight.seq))
#find the PI of these simluated values
height.PI <- apply(sim.height, 2, PI, prob=0.89)
```
...
It's doing a bad job on the tails. The residuals plot would suggest that the relationship is nonlinear (negative residuals on the sides, positive residuals in the middle)

I need to take the time to fiddle with the priors to see if I can substantively change anything here. Not sure what to think of that...
...
precis(m4.5)
```
beta is weird now, I can't get an intuition for it. I know that -23 + 47.08 * weight = predicted value of height.
...
There was a lot more discussion in the lectures that I missed from the book about the importance of simulating the prior predictive distributions. 
for example:
```{r}
set.seed(2971)
N <- 100
a <- rnorm(N, 178, 20)
b <- rnorm(N, 0, 10)
#the plot in lecture 3 is centered on the mean weight of the d2 data, which is = 45
a <- a - b *mean(d2$weight)

plot(c(30,70), c(-100,400), type = "n", xlab = "weight", ylab = "height")
for (i in 1:100)
  abline( a=a[i], b=b[i], col=col.alpha("black", 0.3))
abline(h = 0) #height cannot be negative
abline(h = 272) #height of the tallest man ever
```
We see that a lot of these prior lines just don't make sense
*People don't shrink with weight
*People can't have height <0 
*People don't grow over 277cm (they could possibly, but this would not be the modeling data to speak to them)

In the book he fits a different distribution mean value using the log normal function, because we know that beta can't be negative (people wont be shorter by weight)
However, there's an interesting thing here about if we were predicting a person's height in time, there is some negative beta for when they get old.
```{r}
b <- rlnorm(1e4, 0, 1)
...
There are some betas from the tail of the distribution, which is totally fine.
